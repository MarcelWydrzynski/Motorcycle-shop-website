...
const AchivmentCard = ({ img, number, text }: AchivmentCardProps) => {
  if (!img || number === undefined || !text) return null;
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-8 px-14 border-2 border-lightGrey rounded-lg group hover:bg-primaryRed duration-100 hover:scale-110">
      <img src={img} alt={img} className="w-24 rounded-full p-5 bg-lightGrey border-black border-2 group-hover:text-white" />
